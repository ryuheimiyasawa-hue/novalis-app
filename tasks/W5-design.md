...
**判断**: スキーマ追加は最小化。初期見積もりではほぼ不要。

候補:
- ✅ **必須**: `messages.citations JSONB` 列追加（`{source_type, source_id, slug, snippet, score}[]` を保存し、引用プレビュー再描画に使う）
- ✅ **必須**: `content_embeddings` の **HNSW or ivfflat index** が migration 001 で適切に貼られているか確認、無ければ追加
- ✅ **必須**: `match_content` RPC 関数の確認（migration 001 終端で言及されているが、実装が完了しているか W5 着手時に確認）
- 検討: `messages.metadata JSONB`（latency, tokens, model などをまとめて保存。`whitelist_decision` と統合してもよい）

migration 004 の最終形は W5 着手時に既存スキーマを `\d` で確認してから決める。
...
| モデル | 次元 | 多言語 | 単価 | 採用 |
|---|---|---|---|---|
| **Gemini text-embedding-004** | 768 | ✅ multilingual | $0.025 / 1M chars | ✅ **採用** |
| Gemini Embedding (gemini-embedding-001) | 768/1536/3072 | ✅ | $0.15 / 1M tok | 高品質、コスト 6 倍。MVP 不要 |
| OpenAI text-embedding-3-small | 1536 | ✅ | $0.02 / 1M tok | 別ベンダー、認証/billing 重複コスト |
| 自前 (bge-m3 等) | 1024 | ✅ | サーバ代 | 運用コスト高、MVP 不要 |

**決定**: **text-embedding-004**。理由:
1. 既存 Gemini インフラ流用（API key / billing / monitoring 統一）
2. 多言語対応 = ja クエリで en/tl 記事もマッチ可能
3. コスト最安: 100 articles × 5 chunks × 500 chars × 3 lang = ~750K chars = $0.02 で再 index 完了
4. 768 次元で pgvector の HNSW がサクサク動く
...
## 18. 経営判断ポイント（要 user 確認、設計書承認時）

| # | 論点 | 推奨 | 代替案 |
|---|---|---|---|
| 1 | embedding model | Gemini text-embedding-004（コスト最安、Gemini インフラ統一） | OpenAI text-embedding-3-small（別ベンダー、認証重複） |
| 2 | 多言語戦略 | 単一空間 multilingual + locale ブースト | locale 別空間（storage 3x、純度高い） |
| 3 | chunk size | 500 chars + overlap 100 | 200 chars (細かい)、1000 chars (粗い) |
| 4 | conversations 保存ポリシー | **永続**（MVP デフォルト）、Phase 2 で 30日自動削除 設定可能 | 30日自動削除を MVP デフォルト |
| 5 | SSE vs JSON 一括 | SSE（標準 LLM UX、UX 大幅向上） | JSON 一括（実装簡単、応答遅い印象） |
| 6 | conversation 続け方 | 既存 active 会話に append、ユーザー手動で「新規」ボタン | 24h 経過で自動新規 |
| 7 | citation インライン or footnote | インラインで `[1]` `[2]`、hover/tap で snippet | footnote 一覧表示 (UX シンプル) |
| 8 | E2E テスト導入タイミング | W5 終端で初導入（Playwright） | W6 課金以降に導入 |
