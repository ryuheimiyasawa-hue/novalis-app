...
### Lesson 30: パイプラインを「バイパスする」分岐は、入口のガードまで一緒に外していないか確認する

**事象**: 2026-08-03、P2-B2 で operator モード時に `/api/chat/send` が AI パイプラインを丸ごとスキップする分岐を書いた。狙いは「classifier も Gemini も RAG も呼ばない、クォータも消費しない」だったが、`processChatStream` を呼ばないということは、その内部にあった **PII 検出と入力長チェックも同時に消える**。素直に書くと「運営が対応中だから」という理由だけで、在留カード番号や電話番号がそのまま `messages` に保存される経路ができていた。設計中に気付いて `screenUserInput` として同期ゲートを切り出し、両方の経路から呼ぶ形にした。

**ハック的回避（やらないこと）**:
- operator 分岐にだけ `detectPii` を書き足す → 判定が 2 箇所になり、PII ルール更新時に必ず片方が取り残される（Lesson 12 と同じ「2 source of truth」）
- 「運営が読むだけだから PII が入っても実害はない」と判断する → 入力ゲートの責務は「誰が読むか」ではなく「何を DB に永続化してよいか」。保持期間・削除要求・弁護士確認の対象が増える
- パイプライン全体を呼んで結果だけ捨てる → Gemini のコストと遅延が発生し、バイパスの意味が消える

**根本対処**:
- パイプラインの前段にあった同期ガード（空文字 / 長さ上限 / PII）を `screenUserInput()` として export し、`preflight()` と operator 分岐の両方から呼ぶ。判定は 1 箇所のまま。
- 「AI を呼ぶか」と「DB に入れてよいか」を別の関心事として分離する。前者はスキップしてよいが、後者は経路によらず必ず通す。

**適用基準**:
- 既存の処理チェーンを条件分岐でスキップする実装を書くときは、**そのチェーンが内部で担っていた検証を列挙する**。「何を呼ばなくなるか」ではなく「何が検証されなくなるか」を数える。
- 特に、入力バリデーション・PII・認可・監査ログがチェーンの内部に埋まっている場合は、スキップ前に外へ切り出す。切り出しは分岐追加とセットで行い、後回しにしない。

---

### Lesson 31: 「push が成功した」はブランチ保護を守った証拠にならない

**事象**: 2026-08-03、PR #19 マージ後に handoff を実態へ更新する際、`git checkout main` した状態のまま `git commit` → `git push` した。push は成功したが、リモートの応答は `Bypassed rule violations for refs/heads/main` で、「PR 経由必須」「必須チェック待ち」の 2 つの保護ルールを**バイパス権限で踏み越えていた**。内容が docs のみだったので実害は無かったが、CI を通さない変更が main に入り、Vercel の本番デプロイまで走った。同じ手癖でコードを push していれば、未検証の変更が本番に出ていた。

**ハック的回避（やらないこと）**:
- 「docs だけだから」で正当化する → 保護ルールは中身で例外を作らない。例外を一度作ると次から判断がぶれる
- `git reset --hard` + force push で無かったことにする → push 済みの main を書き換えるのは破壊的で、他の作業者と Vercel のデプロイ履歴を壊す（Lesson 21 の「push 前なら reset、push 後は別対応」と同じ線引き）
- bypass 権限を持っている＝使ってよい、と解釈する → 権限は緊急時の逃げ道であって既定の経路ではない

**根本対処**:
- 既に push 済みの分はそのまま残し、**後続の変更から正規の経路（feature ブランチ → PR → CI）に戻す**。本 Lesson 自体もブランチと PR 経由で入れた。
- `git commit` の前に `git branch --show-current` を見る癖をつける。特に **PR マージ直後は `git checkout main` した直後**なので、この事故が最も起きやすい瞬間。
- push の出力を読む。`Bypassed rule violations` は成功メッセージの中に混ざって出るので、exit code だけ見ていると気付けない。

**適用基準**:
- ブランチ保護のあるリポジトリで作業しているとき、コミット前に必ず現在のブランチを確認する。main なら、まずブランチを切る。
- 「PR マージ後に main へ戻って作業を続ける」流れに入ったら、次の変更を書き始める前にブランチを切る。
- push 出力に `Bypassed` / `protected branch` が出たら、成功していても手を止めて報告する。

---

### Lesson 32: 既存の列に新しい値を「実際に流し始めた」ら、その列を読む側を全部数える

**事象**: 2026-08-03、P2-B2 で `messages.role='operator'` の行を初めて実データとして書き込むようにした。書き込み側は設計書 10 項目を通し、self-audit の 5 役割も回し、`chat/page.tsx` の operator→assistant 暫定マップ（表示側の読み取り）も潰した。それでも **`loadConversationHistory` が `.in("role", ["user","assistant"])` で operator を除外している**ことを見落とし、AI が運営の発言を一切知らないまま会話を再開する状態でリリースした。運営が「この書類を持って窓口へ」と案内した直後に AI が矛盾した一般論を返しうる。

見落としの構造は「role は 001 から 4 値で定義済みだった」こと。カラムもテーブルも増やしていないので、データモデル変更としての警戒が働かなかった。実際には**その値が初めて実在するようになった**時点で、その列を読む全経路の前提が変わっている。

**ハック的回避（やらないこと）**:
- 「表示は直したから読み取り側は済んだ」と考える → 読み取り側は表示だけではない。AI 履歴・分類器・集計・エクスポート・検索がそれぞれ独立に role を解釈している
- operator を無印で assistant として履歴に混ぜる → AI が「自分の過去の発言」として個別助言を見て口調を引き継ぐ。安全性の境界が静かに溶ける（採用したのは印付きで含める案）

**根本対処**:
- 該当列を読んでいる箇所を機械的に列挙してから設計を締める。今回なら `grep -rn "role" src/` で表示・履歴ローダー・分類器・admin 一覧が出る。
- 設計書の「データモデル」節に、**カラム追加が無くても「初めて実在する値」があるなら読み取り経路の棚卸しを書く**。

**適用基準**:
- CHECK 制約や enum に既に存在するが実データが無かった値を、実際に書き込み始めるとき。「スキーマは変わっていない」は棚卸しを省く理由にならない。
- self-audit の役割3（引き継ぎ者）で「この列を読んでいるのは何箇所か」を必ず 1 度数える。表示側を直した時点で満足しない。

---

### Lesson 33: 「チェックが落ちていない」と「チェックが走った」を区別する

**事象**: 2026-08-03、PR #25 の上に PR #26 を積んだ（base を feature ブランチにした）。CI のチェック一覧を見ると Vercel の 2 つだけが並び、赤は 1 つも無い。だが `lint / typecheck / test / build` は**一度も実行されていなかった**。`.github/workflows/ci.yml` のトリガーが `pull_request: branches: [main]` で、main を base にしない PR は対象外だったため。

見た目は「全部緑」と区別がつかない。むしろ audit の赤が消えるぶん、いつもより健全に見える。ローカルで検証していなければ、コンパイルすら通っていないコードをマージしていた。

**ハック的回避（やらないこと）**:
- 「Vercel が pass しているから大丈夫」と読む → Vercel はビルドするが、lint も typecheck も単体テストも走らせない
- 積んだ PR だけ手元で確認して済ませる → 次に誰かが積んだときに同じ穴に落ちる。仕組みで塞がっていない
- base を main に付け替えて誤魔化す → 差分に下の PR のコミットが全部乗り、レビュー不能になる

**根本対処**:
- `pull_request` の `branches` フィルタを外し、**base が何であれ全 PR をゲートする**。積んだ PR に CI 時間を払うほうが、どのジョブもコンパイルしていないコードをマージするより安い。
- `push` 側の `branches: [main]` は残す（feature ブランチへの push ごとに二重で走らせない）。

**適用基準**:
- **チェック一覧が短いときは疑う。** 「赤が無い」ではなく「期待したジョブ名が並んでいるか」を見る。ジョブが 1 つ消えているのは、赤より発見しにくい。
- PR の base を main 以外にしたときは、CI が本当に走ったかを一度だけ確認する。
- ワークフローに `branches` / `paths` フィルタを足すときは、「このフィルタで走らなくなる正当なケースはあるか」を必ず自問する。Lesson 29（report-only の赤を読まない）と同族で、**根は「シグナルの不在を安全と読む」こと**。
