...
- ~~**PR #12 `phase2/p2m-inquiry-slack-notify`**~~ → **2026-07-21 にマージ済み。この記述が誤りだった**（2026-08-03 に `gh pr view` で確認）。コードは既に本番にある。`SLACK_INQUIRY_WEBHOOK_URL` を Vercel 本番に設定するだけで有効化される。未設定なら no-op。

**2026-08-03 時点でオープンな PR はゼロ。**
...
4. **飲食店・専門家の実データ投入**。専門家データは P2-N（マッチング）の前提。**未了で正しい**（2026-08-03 実測: 全件が 2026-07-01 作成の「（サンプル）」データ）。

   **専門家のサンプルは 2026-08-04 に非公開化済み**（ユーザー判断で実施）。`experts` の「（サンプル）」4 件を `is_active = false` にした。行は 5 件とも残してあるので、デモで見せたくなればフラグを戻すだけ。理由は、エスカレーションが専門家一覧へ誘導する設計で、4 件中 3 件に `calendar_url` があり、在留資格で困っている利用者が架空の弁護士の予約ボタンに当たる状態だったため。「（サンプル）」の注記は日本語で、対象である en / tl の利用者には読めない。

   結果として `/api/experts` は 0 件を返す。エスカレーションカードは `experts.length === 0` で見出しと一覧を隠す作り（commit `24f9675`）なので壊れず、本文と「Novalis サポートに問い合わせる」ボタン（`/contact`）だけが残る。**架空の専門家に誘導する代わりに、実在して `/admin/inquiries` で監視されている問い合わせフォームへ誘導される**ので、以前より安全な状態になっている。

   ⚠️ **飲食店のサンプル 5 件は公開したまま**（ユーザー判断。カタログ画面のデモに使うため）。こちらも「（サンプル）」は日本語表記なので、en / tl の利用者には実在の店に見える。ベータテスターを入れる前に判断すること。下げる場合は `UPDATE restaurants SET is_active = false WHERE is_active AND name LIKE '%（サンプル%'`。

   実データ投入は引き続き未了。専門家データは P2-N（マッチング）の前提。
5. **弁護士監修の回答待ち → 2026-08-08〜09（今週末）到着予定**（2026-08-03 にユーザーより）。軽量版パッケージ（`~/Desktop/Novalis弁護士監修依頼_軽量版.docx`、質問 8 問）送付済み。回答でエスカレ文言・PII 方針・4 本柱の可否が確定する。到着したらまとめて解ける項目: P2-L 改善1・4、`NEXT_PUBLIC_ESCALATION_SHOW_CONTINUE_BUTTON` の ON 判断、operator メッセージと会話要約の PII 保持方針、利用規約・プラポリの確定。
...
4. operator 介入の残タスク（優先度低）: 対応すべき会話の通知（エスカレ発生時に Slack へ、PR #12 の仕組みを流用。PR #12 は既にマージ済みなので流用元は本番にある）、`updates` エンドポイントのレート制限。

### このファイルの信頼度について（2026-08-03）

1 セッションで、記述が実態と食い違っていた項目が **4 件**見つかった。Sentry DSN（実は本番に設定済み）、会話の削除・リネーム・検索（実は実装済み）、会話タイトル AI 自動生成（実は実装済み）、PR #12（実は 2026-07-21 にマージ済み）。いずれも「未了」と書かれていたものが実は完了していたパターンで、そのまま信じて着手しかけた。

**このファイルを着手判断の根拠にしないこと。** 実装済みかどうかはコードを grep する、本番の状態は Supabase MCP か curl で実測する、PR の状態は `gh pr view` で見る。Lesson 27 の「repo・handoff・設計書はいずれも陳腐化しうる前提で実機を真実とする」は、このファイル自身にも適用される。
...
**注意（2026-08-03 棚卸し）**: 下のリストは 2026-05 時点のもので、既に実装済みの項目が混ざっている。実測で確認した現況は次のとおり。**着手前に必ずコードを確認すること**（このリストを信じて着手しかけた項目が 2 件あった）。

実装済み: メッセンジャー対応（P2-K / PR #16）、オペレーター介入 UI（P2-B2 / PR #19）、飲食店カタログの管理画面（P2-J / PR #10、実データは未投入）、会話タイトル AI 自動生成（`lib/chat/title.ts`）、**会話の削除・リネーム・検索**（sidebar に実装済み、`conversation-list.tsx` ＋ `PATCH`/`DELETE /api/chat/conversations/[id]`）、問い合わせ first-party 化（P2-M / PR #11）、Supabase migration の未適用検知（P0-B / PR #21、CLI 移行ではなく別方式）、`function_search_path_mutable` 解消（008）、Sentry の PII フィルタと persist 失敗アラート（P1-D）。

データ待ちで着手不可: 月次サンプリングレビュー UI と AI 出力の audit batch（`whitelist_decision` が 185 メッセージ中 5 件しか無く、レビュー対象が存在しない）、P2-N 専門家マッチング（実データ待ち）。

未着手で外部依存なし: 長会話の要約（現状は直近 10 turns 固定で、それ以前は黙って落ちる。operator 発言も同じ 10 枠を食う）、動画埋め込み拡張、起動時 schema assertion（部分適用の穴に効く、P0-B では塞げていない）、Gemini コスト監視（Sentry の DSN が本番に入っていることを確認したので依存は解消済み）、Komoju 課金（大型）。
