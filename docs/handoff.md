...
4. **飲食店・専門家の実データ投入**。専門家データは P2-N（マッチング）の前提。**未了で正しい**（2026-08-03 実測: 全件が 2026-07-01 作成の「（サンプル）」データ）。
   ...
   ⚠️ **ただしそのサンプルが本番で公開状態になっている**。飲食店 5 件・専門家 4 件が `is_active = true`。「（サンプル）」という注記は**日本語なので en / tl の利用者には読めない**。特に専門家は 4 件中 3 件に `calendar_url` があり、エスカレーションで本気の相談を振られた利用者が架空の弁護士と架空の予約リンクに当たる。ベータテスターを入れる前に `is_active = false` にするか、実データに差し替えること。判断はユーザー（デモ用に意図して置いている可能性があるため未対応のまま残してある）。
